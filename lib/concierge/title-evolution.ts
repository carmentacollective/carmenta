...
<title-format>
${MAX_TITLE_LENGTH} character maximum. Recency-weighted: recent topics get specificity, earlier topics consolidate. Emoji at start when it adds instant recognition.

Single topic: "🔧 Fix authentication bug"
...
Decision: { "action": "update", "title": "🗄️ Database migration" }
        ...
        async (span) => {
            try {
                ...
                if (validated.action === "update" && validated.title) {
                    cleanTitle = validated.title.trim();
                ...
                logger.error(
                    { error: errorMessage, currentTitle },
