...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (
            ...
            //
            // Note: We cast writer.write() calls because createUIMessageStream's writer
            // type doesn't include tool-* parts (those are normally generated by streamText,
            // not written manually). The runtime accepts these parts - they're the same
            // format streamText produces. The types are just overly restrictive for manual use.
            const stream = createUIMessageStream({
                execute: ({ writer }) => {
                    // Cast to bypass type restrictions on manual tool part writing
