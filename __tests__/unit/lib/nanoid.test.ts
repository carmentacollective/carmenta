...
describe("generateSlug", () => {
    it("generates slug from title and ID", () => {
        const slug = generateSlug("Fix authentication bug", "a1b2c3d4");
        expect(slug).toBe("fix-authentication-bug-a1b2c3d4");
    });

    it("handles null title", () => {
        const slug = generateSlug(null, "a1b2c3d4");
        expect(slug).toBe("connection-a1b2c3d4");
    });

    it("handles undefined title", () => {
        const slug = generateSlug(undefined, "a1b2c3d4");
        expect(slug).toBe("connection-a1b2c3d4");
    });

    it("handles empty string title", () => {
        const slug = generateSlug("", "a1b2c3d4");
        expect(slug).toBe("connection-a1b2c3d4");
    });
    ...
    it("converts to lowercase", () => {
        const slug = generateSlug("FIX Auth BUG", "a1b2c3d4");
        expect(slug).toBe("fix-auth-bug-a1b2c3d4");
    });

    it("replaces spaces with hyphens", () => {
        const slug = generateSlug("multiple   spaces   here", "a1b2c3d4");
        expect(slug).toBe("multiple-spaces-here-a1b2c3d4");
    });

    it("removes special characters", () => {
        const slug = generateSlug("What's the bug? Fix it!", "a1b2c3d4");
        expect(slug).toBe("whats-the-bug-fix-it-a1b2c3d4");
    });

    it("truncates very long titles", () => {
        const longTitle = "a".repeat(100);
        const slug = generateSlug(longTitle, "a1b2c3d4");
...
describe("extractIdFromSlug", () => {
    it("extracts ID from slug with title", () => {
        const id = extractIdFromSlug("fix-auth-bug-a1b2c3d4");
        expect(id).toBe("a1b2c3d4");
    });

    it("extracts ID from slug without title", () => {
        const id = extractIdFromSlug("connection-a1b2c3d4");
        expect(id).toBe("a1b2c3d4");
    });

    it("extracts ID from ID-only slug", () => {
        const id = extractIdFromSlug("a1b2c3d4");
        expect(id).toBe("a1b2c3d4");
    });
